...
cpMatrix3 cpMatrix3CreateRotation(const cpVector3 direction) {
...
cpMatrix3 cpMatrix3Transpose(const cpMatrix3 A) {
    return (cpMatrix3) {
        A.xx, A.yx, A.zx,
        A.xy, A.yy, A.zy,
        A.xz, A.yz, A.zz,
...
cpMatrix4 cpMatrix4CreateScalar(const fix16_t value) {
    return (cpMatrix4) {
        value,      0,          0,          0,
        0,          value,      0,          0,
        0,          0,          value,      0,
...
cpMatrix4 cpMatrix4CreateRotation(const cpVector3 direction) {
    const cpVector3 right = cpVector3Normalize(cpVector3CrossProduct((cpVector3) {0, fix16_one, 0}, direction));
    const cpVector3 up = cpVector3CrossProduct(direction, right);
...
cpMatrix4 cpMatrix4Transpose(const cpMatrix4 A) {
    return (cpMatrix4) {
        A.xx, A.yx, A.zx, A.wx,
        A.xy, A.yy, A.zy, A.wy,
        A.xz, A.yz, A.zz, A.wz,
