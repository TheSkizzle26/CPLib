...
void cpDefaultFontInit() {
    ...
    const uint32_t defaultFontData[512] = {
    ...
    const int charsWidth[224] = { 3, 1, 4, 6, 5, 7, 6, 2, 3, 3, 5, 5, 2, 4, 1, 7, 5, 2, 5, 5, 5, 5, 5, 5, 5, 5, 1, 1, 3, 4, 3, 6,
...
void cpDrawText(const char* text, const int startX, const int startY, const int fontSize, const cpColor tint) {
    ...
    for (int i = 0; i < charCount; i++) {
        ...
        const int glyphIdx = cur - currentFont.glyphStart;
        if (glyphIdx < 0 || glyphIdx >= currentFont.glyphCount)
            continue;
